...
export const GridImportExport: React.FC<GridImportExportProps> = ({
  height, width, grid, loadInput, setLoadInput, onCopyClick, onLoadGrid, onShareToX
}) => {
  return (
    <>
      <Stack direction="row" spacing={2} mt={4} alignItems="center">
        ...
        <Tooltip title="現在のグリッドを競プロ形式でクリップボードにコピーします (Ctrl+C)">
          <Button variant="outlined" onClick={onCopyClick}>コピー</Button>
        </Tooltip>
      </Stack>
      ...
      <Stack direction="column" spacing={2} mt={4}>
        ...
        <Tooltip title="入力されたテキストを解釈してグリッドに反映します">
          <Button variant="contained" onClick={onLoadGrid}>読み込み</Button>
        </Tooltip>
        <Stack direction="row" justifyContent="flex-end">
          <Tooltip title="現在のグリッドをXでシェアします">
            <IconButton onClick={onShareToX}><XIcon /></IconButton>
          </Tooltip>
        </Stack>
      </Stack>
    </>
